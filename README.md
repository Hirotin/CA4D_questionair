...
固定動画もランダム動画も、設定上はこれまで通り `url` を使います。YouTube にしたい場合は、その `url` を共有URLへ置き換えるだけです。
...
回答を送信すると、その送信分だけの CSV が `SUB-...csv` という名前でブラウザから各端末へ自動ダウンロードされます。同じ CSV が、SMTP 設定済みならメール添付としても送信されます。
